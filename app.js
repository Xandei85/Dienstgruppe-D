...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Legende: nur die Basis-Codes wie in der ursprünglichen Version
  const codes = [
    { code: 'U',    label: 'U' },
    { code: 'AA',   label: 'AA' },
    { code: 'AZA',  label: 'AZA' },
  ...
  function renderGrid(namesArr, container, valueMap) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      // Wochenend- und Feiertagsklassen für Kopfzeile
      const hclasses = [];
      if (dt.getDay() === 6) hclasses.push('sat');
      if (dt.getDay() === 0) hclasses.push('sun');
      if (isHoliday(dt)) hclasses.push('ferienday');
  ...
  function handleCell(cell) {
    ...
    // Keine Overrides für Gelb/Weiß mehr – die alten Codes W2Y/Y2W sind entfernt
