...
ensureNames();
const remarkDay=null, remarkText=null, remarkApply=null;NAMES.forEach(n=>{const o=document.createElement('option');o.value=n;o.textContent=n;meSelect.appendChild(o)});let myName=NAMES[0];meSelect.value=myName;meSelect.addEventListener('change',()=>{myName=meSelect.value});
let names=[...NAMES];
...
document.getElementById('remarkApply')?.click();}});
function toast(m){toastEl.textContent=m;toastEl.classList.add('show');setTimeout(()=>toastEl.classList.remove('show'),1400)}
function render(){try{selectedCode=null;document.querySelectorAll('.legend-btn').forEach(b=>b.classList.remove('active'));
...
if('serviceWorker'in navigator){navigator.serviceWorker.register('sw.js')}catch(e){ console.error('Render-Fehler:', e); gridEl.innerHTML='<div style="padding:8px;color:#900">Fehler beim Rendern: '+e.message+'</div>'; }render();
