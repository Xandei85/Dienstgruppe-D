...
const monthTitle=document.getElementById('monthTitle');const gridEl=document.getElementById('grid');const monthSelect=document.getElementById('monthSelect');const yearSelect=document.getElementById('yearSelect');const prevBtn=document.getElementById('prevMonth');const nextBtn=document.getElementById('nextMonth');const remarksTA=document.getElementById('monthlyRemarks');const saveRemarksBtn=document.getElementById('saveRemarks');const toastEl=document.getElementById('toast');const meSelect=document.getElementById('meSelect');const remarkDay=null,remarkText=null,remarkApply=null;
...
function pushUndo(o){undoStack.push(o);if(undoStack.length>500)undoStack.shift();redoStack.length=0;}
function applyCellState(day,name,value){const td=gridEl.querySelector(`.editable[data-name="${name}"][data-day="${day}"]`);if(!td)return;td.textContent=value||'';}
const CODE_HOTKEYS={'u':'U','a':'AA','z':'AZA','g':'GV','l':'LG','p':'PE','6':'AZA6','2':'AZA12','w':'W2Y','shift+w':'Y2W','backspace':'X','b':'BEER'};
function normKey(ev){const k=ev.key.toLowerCase();return(ev.shiftKey&&k!=='shift')?`shift+${k}`:k}
document.addEventListener('keydown',ev=>{if(ev.ctrlKey&&ev.key.toLowerCase()==='z'){ev.preventDefault();const a=undoStack.pop();if(!a)return;redoStack.push(a);applyCellState(a.day,a.name,a.prev);saveCell({year:curYear,month:curMonth+1,day:a.day,name:a.name,value:a.prev}).catch(()=>{});return;}if(ev.ctrlKey&&ev.key.toLowerCase()==='y'){ev.preventDefault();const a=redoStack.pop();if(!a)return;undoStack.push(a);applyCellState(a.day,a.name,a.next);saveCell({year:curYear,month:curMonth+1,day:a.day,name:a.name,value:a.next}).catch(()=>{});return;}const nk=normKey(ev);const code=CODE_HOTKEYS[nk];if(!code)return;ev.preventDefault();document.querySelectorAll('.legend-btn').forEach(b=>b.classList.remove('active'));const btn=document.querySelector(`.legend-btn[data-code="${code}"]`);if(btn)btn.classList.add('active');selectedCode=code;});function toast(m){toastEl.textContent=m;toastEl.classList.add('show');setTimeout(()=>toastEl.classList.remove('show'),1400)}
function render(){remarksTA.value='';selectedCode=null;document.querySelectorAll('.legend-btn').forEach(b=>b.classList.remove('active'));monthTitle.textContent=`${MONTHS_DE[curMonth]} ${curYear}`;const days=new Date(curYear,curMonth+1,0).getDate();let html='<table><thead><tr>';html+='<th class="name">Name</th>';for(let d=1;d<=days;d++){const dt=new Date(curYear,curMonth,d);const wd=dt.getDay();const wdIdx=(wd+6)%7;const isSa=wdIdx===5,isSo=wdIdx===6;const dayClasses=[];if(inHolidays(dt))dayClasses.push('ferienday');html+=`<th class="${dayClasses.join(' ')}"><div class="daynum">${d}</div><div class="weekday ${isSa?'sat':isSo?'sun':''}">${WD[wdIdx]}</div></th>`}html+='</tr></thead><tbody>';html+='<tr class="day-remarks"><td class="name">Bemerkungen (Tag)</td>';for(let d=1;d<=days;d++){html+=`<td class="day-remark-cell" data-day="${d}"></td>`}html+='</tr>';NAMES.forEach(name=>{html+=`<tr><td class="name">${name}</td>`;for(let d=1;d<=days;d++){const dt=new Date(curYear,curMonth,d);const cls=isWorkday(dt)?'yellow':'';html+=`<td class="editable ${cls}" contenteditable data-name="${name}" data-day="${d}"></td>`}html+='</tr>'});html+='</tbody></table>';gridEl.innerHTML=html;
Promise.all([loadMonth({year:curYear,month:curMonth+1}),loadRemarks({year:curYear,month:curMonth+1}),loadOverrides({year:curYear,month:curMonth+1})]).then(([rows,monthRemark,ovr])=>{
...
if(!d){toast('Tag fehlt');return}const t=remarkText.value||'';const cell=gridEl.querySelector(`.day-remark-cell[data-day="${d}"]`);if(cell)cell.textContent=t;saveDayRemark({year:curYear,month:curMonth+1,day:d,text:t}).then(()=>toast('Tagesbemerkung gespeichert')).catch(()=>toast('Fehler'))});
