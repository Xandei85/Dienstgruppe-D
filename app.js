...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderGrid(namesArr, container, valueMap) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      // Klassen nur für das daynum-Element, nicht für das gesamte th
      const daynumClasses = [];
      if (dt.getDay() === 6) daynumClasses.push('sat');
      if (dt.getDay() === 0) daynumClasses.push('sun');
      if (isHoliday(dt)) daynumClasses.push('ferienday');
      const dnClass = daynumClasses.length > 0 ? ` class="${daynumClasses.join(' ')}"` : '';
      html += `<th><span class="daynum"${dnClass}>${d}</span><br/><span class="weekday">${wd}</span></th>`;
    }
    html += '</tr></thead><tbody>';
    // Zeilen
