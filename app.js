...
function rebuildMeSelect(){ meSelect.innerHTML=''; names.forEach(n=>{const o=document.createElement('option');o.value=n;o.textContent=n;meSelect.appendChild(o)}); if(!names.includes(myName)) myName=names[0]; meSelect.value=myName; }
rebuildMeSelect(); let myName=names[0]; meSelect.value=myName; meSelect.addEventListener('change',()=>{myName=meSelect.value});
