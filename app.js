...
// Ziel: Optik/Design unverändert lassen (gelbe Spalten, Kopf-Farben etc. bleiben wie gehabt)
...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    const toastEl = $id('toast') || (() => {
      // Fallback: kleines Toast-Element dynamisch anlegen, falls es fehlt
    ...
    console.log('[DG-D] app.js loaded');

    if (!gridMain || !legendTop || !meSelect || !monthSelect || !yearSelect || !prevBtn || !nextBtn || !remarksTA || !saveRemarksBtn) {
      console.error('[DG-D] Missing required DOM elements', {
        gridMain: !!gridMain, legendTop: !!legendTop, meSelect: !!meSelect,
        monthSelect: !!monthSelect, yearSelect: !!yearSelect,
        prevBtn: !!prevBtn, nextBtn: !!nextBtn, remarksTA: !!remarksTA, saveRemarksBtn: !!saveRemarksBtn
      });
      // Wenn Elemente fehlen: Seite nicht "leer" machen, aber Hinweis geben
      alert('Fehler: Seite ist nicht vollständig geladen (fehlende HTML-Elemente). Bitte einmal STRG+F5 und ggf. prüfen, ob index.html unverändert ist.');
      return;
    }
    ...
    // "Standard-Extras" (werden nur genutzt, wenn sie in Supabase aktiv sind ODER wenn gar kein Mitarbeiter-Feature verwendet wird)
    const DEFAULT_EXTRA = ['Praktikant', 'Bullen Kate'];

    // aktuell geladene Namen (aktive Mitarbeiter)
    let currentNames = [];
    ...
    // -------------------------------
    // Ferien Bayern (grobe Annäherung wie zuvor)
    ...
    // ---- Legend Codes (inkl. neue Buttons)
    const codes = [
      { code: 'N', label: 'N' },
      { code: 'F', label: 'F' },
      { code: 'S', label: 'S' },
    ...
    // ---- Supabase client (robust)
    function getSupabaseClient() {
      return window.supabaseClient || window.supabase || window._supabase || null;
    }

    // ---- Mitarbeiter laden: wenn Supabase verfügbar und Tabelle existiert
    async function loadActiveEmployees() {
      const client = getSupabaseClient();
      if (!client || !client.from) {
        // Fallback: nur config + default extras
    ...
    async function loadAndRender() {
      ...
      // 5) Render: eine Tabelle
      renderGrid(currentNames, gridMain, valueMap);

      // alte zweite Tabelle leer machen (nur falls vorhanden)
      if (gridExtra) gridExtra.innerHTML = '';
    }

    // ---- Render grid (Design bleibt wie vorher)
    function renderGrid(namesArr, container, valueMap) {
      ...
      (namesArr || []).forEach(name => {
        html += '<tr>';
        // Name-Zelle anklickbar: setzt Dropdown (rein Komfort, Design bleibt gleich)
        ...
        for (let d = 1; d <= daysInMonth; d++) {
          ...
          // Grundfarbe Gelb: bei "Bullen Kate" wie vorher KEINE Standard-Gelb-Färbung
    ...
    // ---- Mitarbeiter Buttons (werden unterhalb Bemerkungen angelegt, ohne das Design der Tabelle zu verändern)
    function ensureEmployeeButtons() {
      // Container: wenn nicht vorhanden -> direkt nach saveRemarksBtn einfügen
      ...
      if (!container) {
        ...
        // Einfügen unterhalb des "Bemerkungen speichern"-Buttons
