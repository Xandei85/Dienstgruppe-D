...
async function saveOverride({ year, month, day, name, yellow_override }) {
  ...
  return postToTable('overrides', row, ['project','year','month','day','name']);
}
async function loadOverrides({ year, month }) {
  if(!sb) return lsLoadOverrides({ year, month });
  return fetchFromTable('overrides', { project: window.APP_CONFIG.PROJECT_NAME || 'Schichtplan', year, month }, '*');
}
